...
export function reviewBridge(photoConfirmed = true): UsBridge {
 ...
 return {
  ...
  async confirmPhoto(file,participantId=reviewPair.sender.id){if(participantId===reviewPair.sender.id){setup.photoUrl=URL.createObjectURL(file);setup.photoConfirmed=true}else{setup.recipientPhotoUrl=URL.createObjectURL(file);setup.recipientPhotoConfirmed=true}return {...setup}},
  ...
  async createJob(input:CreateJob){const job:Job={...input,id:crypto.randomUUID(),storyId:crypto.randomUUID(),status:'generating',createdAt:new Date().toISOString()};jobs=[job,...jobs];finish(job.id);return {...job}},
