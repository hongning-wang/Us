<script lang="ts">
 ...
 async function choose(event:Event){
  const upload=event.target as HTMLInputElement;const file=upload.files?.[0];upload.value='';if(!file||busy)return;requestVersion++;loading=false;busy=true;
...
<div class="backdrop" role="presentation" onclick={(e)=>{if(e.target===e.currentTarget&&!busy)onclose()}}>
 <div bind:this={dialog} class="panel" role="dialog" aria-modal="true" aria-label={settings?'Us settings':target===pair.sender.id?'Your photo':'Friend photo'} tabindex="-1" onkeydown={keydown}>
  <header><div><h1>{settings?'Us settings':'Your photos'}</h1><p>{settings?'Photos for this chat':'@'+person.username}</p></div><button class="close" aria-label="Close Us settings" disabled={busy} onclick={onclose}>×</button></header>
